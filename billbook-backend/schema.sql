-- =============================================
-- BILLBOOK DATABASE SCHEMA
-- =============================================
CREATE DATABASE IF NOT EXISTS billbook CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE billbook;
...
CREATE TABLE IF NOT EXISTS bb_users (
    ...
    role ENUM('OWNER','STAFF','ACCOUNTANT') DEFAULT 'OWNER',
...
CREATE TABLE IF NOT EXISTS bb_invoices (
    ...
    invoice_type ENUM('SALE','PURCHASE','SALE_RETURN','PURCHASE_RETURN') DEFAULT 'SALE',
    ...
    payment_mode ENUM('CASH','UPI','CARD','BANK_TRANSFER','CHEQUE','CREDIT') DEFAULT 'CASH',
    notes TEXT,
    terms TEXT,
...
CREATE TABLE IF NOT EXISTS bb_khata_entries (
    ...
    entry_type ENUM('CREDIT_GIVEN','CREDIT_RECEIVED','DEBIT_GIVEN','DEBIT_RECEIVED','PAYMENT_RECEIVED','PAYMENT_MADE','INTEREST') DEFAULT 'CREDIT_GIVEN',
    amount DECIMAL(12,2) NOT NULL,
...
CREATE TABLE IF NOT EXISTS bb_cash_register (
    ...
    FOREIGN KEY (user_id) REFERENCES bb_users(id) ON DELETE CASCADE,
    UNIQUE KEY uk_user_date (user_id, register_date)
);
...
CREATE TABLE IF NOT EXISTS bb_quick_cash_entries (
    ...
    entry_type ENUM('CASH_SALE','UPI_SALE','CASH_RECEIVED','CASH_PAID_OUT','EXPENSE','SHORT_PAYMENT','ADJUSTMENT') DEFAULT 'CASH_SALE',
    amount DECIMAL(12,2) DEFAULT 0.00,
    payment_mode ENUM('CASH','UPI','CARD','BANK_TRANSFER','CHEQUE') DEFAULT 'CASH',
    description VARCHAR(500),
